...
def url_pairs_to_remove(args, bucket_urls, url_doc):
    ...
    for main_bucket_idx, main_url in enumerate(bucket_urls):
        if main_bucket_idx in removed_indices:
            continue
        ...
        for other_bucket_idx in range(0, len(items)):
            if other_bucket_idx in removed_indices:
                continue
            counter_local += 1
            other_url = items[other_bucket_idx]
            # if other_url == main_url:
            if other_bucket_idx == main_bucket_idx:
                continue
            ...
            if jaccard_sim > 0.5:
                ...
                removed_indices.append(other_bucket_idx)

        # bucket_urls.remove(main_url)
        removed_indices.append(main_bucket_idx)
...
def handle_bucket(bucket_id, each_bin, args, url_doc):
    bucket_urls = each_bin[bucket_id]
    ...
    return remove_urls_list_sub, deduped_local_sub, counter_local_sub
...
def compute_jaccard(each_bin, num_bins, start_time_local):
    ...
    print("len each bin:", len(each_bin))

    # pool = multiprocessing.Pool(min(num_bins, args.max_workers_jaccard))
    pool = multiprocessing.Pool(args.max_workers_jaccard)

    handle_bucket_partial = partial(handle_bucket, each_bin=each_bin, args=args, url_doc=url_doc)
    handle_bucket_iter = pool.imap(handle_bucket_partial, each_bin, chunksize=2048)
    # handle_bucket_iter = pool.map(handle_bucket_partial, each_bin, chunksize=128)

    for remove_urls_list_sub, deduped_local_sub, counter_local_sub in handle_bucket_iter:
...
def find_pair_urls_parallel(args, lshcache, url_doc):
    ...
    # compute jaccards of buckets in bin in parallel (parallelism limited to # of bins)
    ...
    compute_jaccard_iter = [compute_jaccard_partial(each_bin) for each_bin in lshcache.bins]
